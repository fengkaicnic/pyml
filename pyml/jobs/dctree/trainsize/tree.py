...
def build_tree(dataset,labels,features,weight):
    ...
    for key in split_feature_dct.keys():
        ...
        decesion_tree[split_feature][key] = build_tree(split_feature_dct[key], train_labs, features, weight)
...
def test(train_file, test_file, rstname):
    decesion_tree = read_tree('prunsize_tree')
    test_dataset,test_features = format_data(test_file)
    result = []
    for data in test_dataset:
        label = classify_t(decesion_tree, test_features, data)
        result.append(label)
    store_tree(result, rstname)
...
#     run(train_file, test_file, 0.057)
    test(train_file, test_file, 'sizeresult.txt')
end = time.clock()
print (end - start)