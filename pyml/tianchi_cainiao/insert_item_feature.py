...
try:
    ...
    conn.commit()
    mode = re.compile(r'\d+')
    with open('e:/tianchi/cainiao/item_feature1.csv') as file:
        lines = file.readlines()

    table_sql = 'select column_name, data_type from information_schema.columns where \
                  table_schema="test" and table_name="item_feature" and column_name != "id"'
    cur.execute(table_sql)
    rst = cur.fetchall()

    for line in lines:
        insertsql = ['insert into item_feature(']
        valuesql = [' values(']
        valuelst = line.split(',')
