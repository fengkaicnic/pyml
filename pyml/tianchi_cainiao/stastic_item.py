...
try:
    ...
    conn.commit()
    sql = 'select distinct(item_id) from config limit 1'
    cur.execute(sql)
    rst = cur.fetchall()
    ...
    for num in range(100):
        ...
        for tid in rst:
            ...
            for lst in tem_dct[tid[0]]:
                lst.append(0)
            termsql = 'select amt_alipay, store_code, num_alipay, date, amt_alipay_njhs, num_alipay_njhs from \
                         item_store_feature where item_id = %d and date > %s and date < %s \
                         ' % (tid[0], s_date, e_date)
            cur.execute(termsql)
            termrst = cur.fetchall()
            for term in termrst:
                tem_dct[tid[0]][term[1]-1][num] += term[4]
        # pdb.set_trace()
