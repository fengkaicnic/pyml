...
try:
    ...
    for term_id in rst:
        ...
        for num in range(1):
            ...
            sql = 'select sum(pv_ipv), sum(pv_uv), sum(cart_ipv), sum(cart_uv), sum(collect_uv) from item_store_feature where store_code = %d and\
                     date > "%s" and date <= "%s" and item_id = %d' % (store_code, st_date, en_date, term_id)
            cur.execute(sql)
            f_rst = cur.fetchall()
#             pdb.set_trace()
    ...
    with open('d:/tianchi/model/test_store_%s.csv' % str(store_code), 'wb') as file:
        file.writelines('\n'.join(result_lst))
