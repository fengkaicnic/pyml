...
def input_data(train_file, test_file):
    ...
    with open(train_file, 'r') as f1:
        for line in f1:
            tks = line.split(',', 1)
            train_words.append(tks[0].decode('gbk'))
            train_tags.append(tks[1])
    with open(test_file, 'r') as f1:
        for line in f1:
            tks = line.split(',', 1)
            test_words.append(tks[0].decode('gbk'))
            test_tags.append(tks[1])
    return train_words, train_tags, test_words, test_tags


with open('d:/naren/stopwords.txt', 'r') as f:
...
def main():
    ...
    train_file = 'd:/naren/train.csv'
    test_file = 'd:/naren/test.csv'
    pdb.set_trace()
