...
def get_feature(cur, feature_lines, flag, pos_id, resume_id):

    sql1 = 'select low_income, description, low_workage, position_name, \
            education from company where position_id = %d' % pos_id
    ...
    sqlp = 'select dessalary, skills, destitle, hisprojects, otherinfo from profile where \
             pos_id = %d and resume_id = %d and recommend = %d and confirm = %d limit 15' % (pos_id, resume_id, flag, flag)
    ...
    for pro in profile:
        ...
        pro_otherinfo = pro[4]
        sql_work = 'select description from work where pos_id = %d and resume_id = %d' % (pos_id, resume_id)
...
try:
    ...
    get_feature(cur, feature_lines, 0, 42325, 3678547)
