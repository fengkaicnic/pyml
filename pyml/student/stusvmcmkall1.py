...
import MySQLdb
try:
    scorearff = open('d:/student/svm/stusvmcmkall1.arff', 'w+')
    ...
    for consumedd in consumeddlsr:
        for consumeddnm in xrange(24):
        #consumekindct[consumedd[0]] = consumecnt
            consumecnt += 1
            consumeddlst.append('@attribute %s-%s numeric \n' % (consumedd[0], str(consumeddnm)))
    bookindct = {}
    i = 0
    ...
    for lbr in librarylsr:
        ...
        consumestudct[lbr[0]] = {}
        for consumekd in consumeddlsr:
            consumestudct[lbr[0]][consumekd[0]]= [0 for i in xrange(24)]
    ...
    consumestuall = [0 for i in xrange(91)]
    pdb.set_trace()
    for consumestu in consumestulsr:
        print consumestu
        if consumestu[2] is None:
            continue
        consumestuall[consumestu[0]-1] += 1
        if len(consumestu[3]) == 6:
            consumestudct[consumestu[0]][consumestu[2]][int(consumestu[3][:2])] += 1
        elif len(consumestu[3]) == 5:
            consumestudct[consumestu[0]][consumestu[2]][int(consumestu[3][:1])] += 1
        else:
            consumestudct[consumestu[0]][consumestu[2]][0] += 1
    for xuehao in xuehaolst:
        ...
        for consumedd in consumeddlsr:
            tmsum = consumestuall[xuehao-1]
            if tmsum != 0:
                consumestudct[xuehao][consumedd[0]] = map(lambda x: float(x)/tmsum, consumestudct[xuehao][consumedd[0]])
                stulslst = stulslst + consumestudct[xuehao][consumedd[0]]
